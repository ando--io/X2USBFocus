...
{
   if (GetLogger()) 
      GetLogger()->out((char *)"X2USBFocus::establishLink");
...
{
   if (GetLogger())
      GetLogger()->out((char *)"X2USBFocus::focMinimumLimit");
...
{
   if (GetLogger())
      GetLogger()->out((char *)"X2USBFocus::focMaximumLimit");
...
{
   if (GetLogger())
      GetLogger()->out((char *)"X2USBFocus::focAbort");
...
int	X2USBFocus::startFocGoto(const int& nRelativeOffset) {
   ...
   if (nRelativeOffset < 0) {
      sprintf(str, "I%05d", abs(nRelativeOffset));
      result = sendRequest(str);
   } else if (nRelativeOffset > 0) {
      sprintf(str, "O%05d", nRelativeOffset);
      result = sendRequest(str);
   }
...
void X2USBFocus::resetDevice(const char * name) {
   ...
   resetUSBDeviceWithBSDPath(str);
   CFRelease(str);
/*
   if (GetLogger())
      GetLogger()->out((char *)"X2USBFocus::resetDevice Willing to reset device");
   OSStatus r;
   
   UsbDeviceEnumerator * enumerator = new UsbDeviceEnumerator(GetLogger());
   enumerator->collectDevices();
   IOUSBDeviceStruct187 ** dev = (IOUSBDeviceStruct187 **)enumerator->deviceWithName(name);
   if (dev && *dev) {
      
      if (GetLogger())
         GetLogger()->out((char *)"X2USBFocus::resetDevice device found");
      
      r = (*dev)->USBDeviceOpen(dev);
      if (r == noErr) {
         if (GetLogger())
            GetLogger()->out((char *)"X2USBFocus::resetDevice device Opened");
         
         r = (*dev)->ResetDevice(dev);
         if (r == noErr) {
            if (GetLogger())
               GetLogger()->out((char *)"X2USBFocus::resetDevice device reset");
            
            
            r = (*dev)->USBDeviceReEnumerate(dev, 0);
//            r = (*dev)->USBDeviceClose(dev);
            if (r == noErr) {
               if (GetLogger())
                  GetLogger()->out((char *)"X2USBFocus::resetDevice device renumerated");
            }
         }
      }
   }
